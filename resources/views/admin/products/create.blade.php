...
<div class="container mx-auto px-4 py-6">
    <h1 class="text-3xl font-bold text-gradient bg-gradient-to-r from-purple-400 via-pink-500 to-red-500 bg-clip-text text-transparent mb-6">Add Product</h1>
    ...
    <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-md space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="price" value="{{ old('price') }}" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400">
            </div>
        </div>

        <div>
            <label class="block mb-1 font-semibold">Description</label>
            <textarea name="description" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400" rows="4">{{ old('description') }}</textarea>
        </div>
        ...
        <div class="mt-4">
            ...
            <div id="variants-container" class="space-y-2">
                <div class="flex gap-2">
                    <input type="text" name="variants[0][sku]" placeholder="SKU" class="border rounded px-2 py-1 w-1/4">
                    <input type="text" name="variants[0][attribute]" placeholder="Attribute" class="border rounded px-2 py-1 w-1/4">
                    <input type="number" step="0.01" name="variants[0][price]" placeholder="Price" class="border rounded px-2 py-1 w-1/4">
                    <input type="number" name="variants[0][stock]" placeholder="Stock" class="border rounded px-2 py-1 w-1/4">
        ...
        <div class="mt-6">
            <button type="submit" class="px-6 py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white font-semibold rounded-lg hover:from-purple-600 hover:to-pink-600 transition">Save Product</button>
        </div>
    </form>
</div>
